...
Musketeer is a local-first CLI execution harness for role-separated AI workflows. All state lives on disk in a `.musketeer/` directory.

### Three-Role Model

Every workflow enforces three isolated roles with explicit handoffs between them:
1. **Originator** — intent definition and planning
2. **Cross-Examiner** — adversarial review and constraint enforcement
3. **Executor** — deterministic execution and artifact production
